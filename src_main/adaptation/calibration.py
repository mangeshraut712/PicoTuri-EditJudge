...
class BinaryCalibrator:
    ...
    def _fit_temperature_scaling(
        ...
        def nll_loss(temp):
            ...
            return -torch.mean(
                targets * torch.log(pos_probs + 1e-10) + (1 - targets) * torch.log(1 - pos_probs + 1e-10)
            )
    ...
    def _calculate_metrics(
        ...
        nll = -np.mean(
            targets * np.log(calibrated_probs + 1e-10) + (1 - targets) * np.log(1 - calibrated_probs + 1e-10)
        )
